/**
 * Rutas de Reportes - Firebase App Hosting (Firestore)
...
router.get('/consolidado', verificarAuth, async (req, res) => {
    try {
        const { estado, departamento, busqueda } = req.query;
        ...
        let solicitudes = [];
        snapshot.forEach(doc => solicitudes.push({ id_solicitud: doc.id, ...doc.data() }));
        ...
        await generarInformeConsolidadoExcel(solicitudes, { estado, departamento }, outputPath);
...
router.get('/historial-correos', verificarAuth, async (req, res) => {
    try {
        ...
        const correos = [];
        snapshot.forEach(doc => correos.push({ id: doc.id, ...doc.data() }));
