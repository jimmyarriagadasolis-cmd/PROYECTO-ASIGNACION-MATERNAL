/**
 * Servicio de Correo Electrónico - Firebase Cloud Functions
...
async function obtenerConfigSMTP() {
    ...
    const configIds = ["smtp_host", "smtp_port", "smtp_user", "smtp_pass"];
...
/**
 * Obtiene el nombre de la institución desde Firestore
 */
async function obtenerNombreInstitucion() {
    const db = admin.firestore();
    const doc = await db.collection("configuracion").doc("nombre_institucion").get();
    return doc.exists ? doc.data().valor : "Ministerio de las Culturas";
}
...
async function enviarFichaIndividual(solicitud, pdfPath, destinatarios) {
    ...
    if (!transporter) {
        await registrarEnvioCorreo(db, solicitud.id_solicitud || solicitud.id, "ficha_individual",
            destinatarios.join(", "), "Ficha de Asignación Maternal", "pendiente", "SMTP no configurado");
        return { success: false, error: "El servidor de correo no está configurado" };
    }

    const nombreInstitucion = await obtenerNombreInstitucion();
    const smtpConfig = await obtenerConfigSMTP();
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin: 15px 0;">
                        ...
                        <tr style="background-color: #edf2f7;">
                            ...
                            <td style="padding: 10px; border: 1px solid #e2e8f0; font-size: 18px; color: #1a365d;">
                                <strong>$${(solicitud.monto_total_pagable || 0).toLocaleString("es-CL")}</strong>
                            </td>
                        </tr>
                    </table>
    ...
    try {
        ...
        await registrarEnvioCorreo(db, solicitud.id_solicitud || solicitud.id, "ficha_individual",
            destinatarios.join(", "), mailOptions.subject, "enviado");
        return { success: true, message: "Correo enviado exitosamente" };
    } catch (error) {
        await registrarEnvioCorreo(db, solicitud.id_solicitud || solicitud.id, "ficha_individual",
            destinatarios.join(", "), mailOptions.subject, "fallido", error.message);
...
async function enviarReporteConsolidado(excelPath, destinatarios, resumen) {
    ...
    if (!transporter) {
        await registrarEnvioCorreo(db, null, "reporte_consolidado",
            destinatarios.join(", "), "Reporte Consolidado", "pendiente", "SMTP no configurado");
        return { success: false, error: "El servidor de correo no está configurado" };
    }
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin: 15px 0;">
                        ...
                        <tr style="background-color: #edf2f7; font-weight: bold;">
                            ...
                            <td style="padding: 12px; border-bottom: 2px solid #1a365d; text-align: right; font-size: 16px; color: #1a365d;">
                                $${(resumen.montoTotal || 0).toLocaleString("es-CL")}
                            </td>
                        </tr>
                    </table>
 ...
 */
async function registrarEnvioCorreo(db, solicitudId, tipo, destinatarios, asunto, estado, error = null) {
