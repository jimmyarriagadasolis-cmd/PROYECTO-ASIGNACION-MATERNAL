/**
 * Rutas de Solicitudes - Versión Firestore (Corregida para async/await en cálculos)
...
router.get('/estadisticas', async (req, res) => {
    try {
        ...
        const stats = {
            porEstado: {},
            totales: { total_solicitudes: solicitudes.length, monto_total_acumulado: 0, total_retroactivos: 0, total_futuros: 0 },
            porDepartamento: {},
            porTramo: {},
        ...
        for (const sol of solicitudes) {
            if (!stats.porEstado[sol.estado_solicitud]) stats.porEstado[sol.estado_solicitud] = { cantidad: 0, monto_total: 0 };
            ...
            if (!stats.porDepartamento[depto]) stats.porDepartamento[depto] = { cantidad: 0, monto_total: 0 };
            ...
            if (!stats.porTramo[tramo]) stats.porTramo[tramo] = { cantidad: 0, monto_total: 0 };
            ...
            if (fechaRegistro >= treintaDiasAtras) stats.solicitudesUltimos30Dias++;
        }
...
router.post('/', async (req, res) => {
    try {
        ...
        // *** CORRECCIÓN CLAVE: Await en el cálculo ***
        ...
        const nuevaSolicitud = {
            ...
            monto_mensual_asignacion: calculo.montoPromedio, // Usar monto promedio
...
router.put('/:id', async (req, res) => {
    try {
        ...
        camposPermitidos.forEach(campo => {
            if (updates[campo] !== undefined) camposActualizar[campo] = updates[campo];
        });
        ...
        // *** CORRECCIÓN CLAVE: Await si se recalcula ***
        if (updates.sueldo_bruto_mensual || updates.fecha_inicio_embarazo || updates.fecha_ingreso_solicitud) {
            ...
            camposActualizar.monto_mensual_asignacion = calculo.montoPromedio;
...
// ... (Rutas DELETE y cambiar-estado no usan el cálculo y no necesitan cambios)
router.delete('/:id', async (req, res) => {
    try {
        ...
        const doc = await docRef.get();
        if (!doc.exists) return res.status(404).json({ success: false, error: 'Solicitud no encontrada' });
        if (doc.data().estado_solicitud !== 'Ingresada') return res.status(400).json({ success: false, error: 'Solo se pueden eliminar solicitudes en estado "Ingresada"' });
        await docRef.delete();
...
router.post('/:id/cambiar-estado', async (req, res) => {
    try {
        ...
        if (!['Aprobada', 'Rechazada', 'En Revisión', 'Pagada'].includes(estado)) return res.status(400).json({ success: false, error: 'Estado no válido' });
        const docRef = db.collection('Solicitudes_Asignacion_Maternal').doc(id);
        const doc = await docRef.get();
        if (!doc.exists) return res.status(404).json({ success: false, error: 'Solicitud no encontrada' });
        const updateData = { estado_solicitud: estado, usuario_aprobacion: usuario_id || 'sistema' };
        if(estado === 'Aprobada') updateData.fecha_aprobacion = FieldValue.serverTimestamp();
        else if (estado === 'Rechazada') updateData.observaciones = `${doc.data().observaciones || ''} [Rechazada: ${motivo || 's/m'}]`.trim();
 ...
 * Previsualizar un cálculo (AHORA ASÍNCRONO)
 */
router.post('/calcular-preview', async (req, res) => { // <-- async
    try {
